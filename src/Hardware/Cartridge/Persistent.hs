...
restoreRamBank (CartridgeRAMSave ram) =
  RamBank 0 $ V.unfoldr splitRamBanks $ byteStringToVector ram
 where
...
saveAgreesWithHeader save header =
  ramBankCount (restoreRamBank save) == fromIntegral (H.headerRamBanks header)
