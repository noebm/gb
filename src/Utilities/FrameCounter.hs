...
setupFrameCounter = do
  ...
  return $ FrameCounter
    ...
    , displayCounter = do
        cursorUp 2
        clearLine
        putStrLn . (printf "30 frame window: %.2f ms") . averageWin =<< readIORef avgWindowFrameTime
        clearLine
        putStrLn . (printf "240 frame window: %.2f ms") . averageWin =<< readIORef avgOverallFrameTime
    }
