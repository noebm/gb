...
keyMotion =
  lens SDL.keyboardEventKeyMotion (\s x -> s { SDL.keyboardEventKeyMotion = x })

keyRepeat :: Lens' SDL.KeyboardEventData Bool
keyRepeat =
  lens SDL.keyboardEventRepeat (\s x -> s { SDL.keyboardEventRepeat = x })

keyWindow :: Lens' SDL.KeyboardEventData (Maybe SDL.Window)
keyWindow =
  lens SDL.keyboardEventWindow (\s x -> s { SDL.keyboardEventWindow = x })

keyKeysym :: Lens' SDL.KeyboardEventData SDL.Keysym
keyKeysym =
  lens SDL.keyboardEventKeysym (\s x -> s { SDL.keyboardEventKeysym = x })
