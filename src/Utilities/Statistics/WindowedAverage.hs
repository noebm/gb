...
emptyWindow size def =
  WindowedAverage { window = fromList $ replicate size def, averageWin = def }

addWindowSample :: Fractional a => a -> WindowedAverage a -> WindowedAverage a
addWindowSample sample win = win
  { window     = window'
  , averageWin = averageWin win + (sample - old) / fromIntegral
                   (lengthCirc (window win))
  }
