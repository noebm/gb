...
parseExtendedInstruction b =
  ...
  in
    case byteCodeDecompose b of
      ...
      (1,y,z) -> o (if z == 6 then 12 else 8) (BIT y $ readable8 $ basicRegisterArg z)
...
parseInstructionM b =
  ...
  in
    ...
    (0,y,1) | y `testBit` 0 -> return $! o 8 $ ADD16_HL $ readable16 (argWritable16 y)
            | otherwise     -> do
                dat <- word
                return $! o 12 $ LD16 (ReadImm16 dat) (argWritable16 y)

    (0,y,2) | y `testBit` 0 -> return $! o 8 $ LD (ReadAddr8 $ registerPointerArg y) (WriteReg8 A)
            | otherwise     -> return $! o 8 $ LD (ReadReg8 A) (WriteAddr8 $ registerPointerArg y)

    (0,y,3) | y `testBit` 0 -> return $! o 8 $ DEC16 $ argWritable16 y
            | otherwise     -> return $! o 8 $ INC16 $ argWritable16 y

    (0,y,4) -> return $! o (if y == 6 then 12 else 4) $ INC (basicRegisterArg y)
    (0,y,5) -> return $! o (if y == 6 then 12 else 4) $ DEC (basicRegisterArg y)
    (0,y,6) -> do
      dat <- byte
      return $! o (if y == 6 then 12 else 8) $ LD (ReadImm8 dat) (basicRegisterArg y)
    ...
    (1,y,z) -> return $! o (if y == 6 || z == 6 then 8 else 4)
      $ LD (readable8 $ basicRegisterArg z) (basicRegisterArg y)

    (2,y,z) -> return $! o (if z == 6 then 8 else 4) (aluMnemonic y $ readable8 $ basicRegisterArg z)
