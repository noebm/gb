...
build :: MonadEmulator m => Word16 -> StateT (M.Map Word16 (Instruction Flag ())) m ()
build addr = do
  lift $ jump addr
  instr <- lift step
  pc <- lift loadPC
  at addr ?= (instr $> ())
  let isUnconditionalRet = hasn't flag instr && (instr ^. expr == RET || instr ^. expr == RETI)
  unless isUnconditionalRet $ do
    ...
    let instr' = case instr of
          InstructionNode addr (CALL addr') -> InstructionBranch Nothing addr () (CALL' addr')
          _ -> instr & flag .~ ()
...
disassembleF :: (Foldable t , MonadEmulator m)
             => t Word16 -> m (M.Map Word16 (Instruction Flag ()))
...
disassembleAt :: MonadEmulator m => Word16 -> m (M.Map Word16 (Instruction Flag ()))
