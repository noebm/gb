...
basicSDLEmulationConfig nodelay = do
  ...
  return $ EmulationConfig
    ...
    , keyUpdate   = do
      ...
      let keys =
            events ^.. folded . _KeyboardEvent . to (updateKeys keymap) . _Just
      let quit =
            has (folded . _QuitEvent) events
              || has (folded . _WindowClosedEvent) events
      return (quit, keys)
    , shouldSave  = True
    }
...
mainloop fp nodelay = do
  ...
  (bootrom', rom) <-
    ...
    $   runExceptT
    $   (,)
    <$> readBootRom bootStrapName
    <*> readRom True fp

  config  <- basicSDLEmulationConfig nodelay
  ramSave <- emulate (Just bootrom') rom config
  mapM_ (B.writeFile (romSaveFilePath rom) . encode)
        (guard (shouldSave config) *> ramSave)
